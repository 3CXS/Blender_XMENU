...
from .brushtexture import texture_path, get_tex_previews, get_brush_tex_previews, get_stencil_tex_previews

def brushmode(self,context):
...
def texmode(self,context):
    mode = context.active_object.mode
    if mode == 'TEXTURE_PAINT':
        tex_mode = 'image_paint'
    if mode == 'SCULPT':
        tex_mode = 'sculpt'
...
def register():
    bpy.types.Scene.xm_brush_mode = StringProperty(default="None")
    bpy.types.Scene.xm_tex_mode = StringProperty(default="None")
